...
if __name__ == "__main__":
    ...
    parser.add_argument('lang',help='language of the text(s)',choices=['sl','hr','sr'],default="examples/example.txt.redi")
    parser.add_argument('path',help='Path to the file or folder containing files (with .txt extension) to be tagged and lemmatised.', default="hr")
    args=parser.parse_args()
    passwd= getpass('Input password for user "'+user+'": ')
    tagger = Tagger(args.lang)
    tagger.authorize(user, passwd)
    if os.path.isfile(args.path):
        write(json.loads(tagger.tagLemmatise(open(args.path).read())),open(args.path+'.taglem','w')) #.decode(coding).encode('utf8')
    else:
        files=[e for e in os.listdir(args.path) if e.endswith('.txt')]
        for index,file in enumerate(files):
            print ('Processing',index+1,'/',len(files))
            write(json.loads(tagger.tagLemmatise(open(os.path.join(args.path,file)).read())),open(os.path.join(args.path,file)+'.taglem','w')) #.decode(coding).encode('utf8')
